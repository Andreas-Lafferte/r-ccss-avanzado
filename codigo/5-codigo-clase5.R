...
# install.packages(c("haven", "tidyverse", "car", "labelled")) # ya tenemos instalados estos paquetes
...
# (*) Pasos de análisis realizados en Code Ocean
...
saveRDS(casen_santiago, file = "data/casen_santiago.RDS")

# ---- Cargar base creada desde Code Ocean
...
names(casen_santiago) # Detectar columnas de interés
...
# Esto todavía se podría automartizar más.
...
#Se usarán variables no estandarizadas (2:6)
...
# Cálculo de distancias (euclídea al cuadrado) con subconjunto de variables
# enfermos, dependientes / dinero, trámites / trabajo
# d <- dist(comunas[c(2:3, 5:6, 9)])^2      # (^2) sobre variables de interés
...
save(comunas, file = "datos/cluster_comunas_casen.RData")